...
async def pincode_not_found_exception_handler(request:Request,exc:pincode_not_found_error):
    return JSONResponse(
        status_code=404,
        content={"message":f"Pincode {exc.pincode} not found."}
    )

async def invalid_pincode_exception_handler(request:Request,exc:invalid_pincde_error):
    return JSONResponse(
        status_code=400,
